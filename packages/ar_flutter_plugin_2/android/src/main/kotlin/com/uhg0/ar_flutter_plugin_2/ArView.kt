...
import com.uhg0.ar_flutter_plugin_2.Serialization.deserializePose
...
class ArView(
    ...
    private val planeTrackingStartedAt = mutableMapOf<Plane, Long>()
    ...
    companion object {
        private const val PLANE_STABILITY_NANOS = 750_000_000L
        private const val MIN_PLANE_EXTENT_METERS = 0.25f
    }
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        ...
        val transformation = (nodeData["transformation"] as? List<*>)
            ...
            ?.map { it.toFloat() }
            ?.toFloatArray()
        if (transformation == null || transformation.size != 16) {
            return null
        }
        ...
        return try {
            sceneView.modelLoader.loadModelInstance(fileLocation)?.let { modelInstance ->
                object : ModelNode(modelInstance = modelInstance) {
                    ...
                    // ARNode serializes a complete column-major Matrix4. The
                    // public bridge treated m00 as scaleToUnits and discarded
                    // translation/rotation, which broke pivot correction and
                    // every transform update.
                    transform = transformation.toTransform()
                    isPositionEditable = handlePans
                    isRotationEditable = handleRotation
                    name = nodeData["name"] as? String
                    val bounds = boundingBox
                    val data = nodeData["data"] as? Map<*, *>
                    Log.i(
                        TAG,
                        "[AR_FIX] original bounds " +
                            "center=${bounds.center.contentToString()} " +
                            "halfExtent=${bounds.halfExtent.contentToString()}",
                    )
                    Log.i(TAG, "[AR_FIX] normalized scale ${data?.get("normalizedScale")}")
                    Log.i(TAG, "[AR_FIX] base correction ${data?.get("baseCorrectionY")}")
                    Log.i(TAG, "[AR_FIX] final transform ${transformation.contentToString()}")
                }
            } ?: run {
                null
            }
    ...
    private fun handleInit(
        ...
        try {
            ...
            sceneView.apply {
                ...
                // Center mode only visualizes HORIZONTAL_UPWARD_FACING planes
                // and avoids rendering every transient plane in the session.
                ...
                onFrame = { _ ->
                    try {
                        if (!isSessionPaused) {
                            // ARSceneView already calls Session.update() once per frame.
                            // Calling it again here can destabilize tracking.
                            sceneView.frame?.let { frame ->
                                val cameraState = frame.camera.trackingState
                                if (cameraState != lastCameraTrackingState) {
                                    lastCameraTrackingState = cameraState
                                    val reason = frame.camera.trackingFailureReason
                                        .takeUnless { cameraState == TrackingState.TRACKING }
                                        ?.name
                                    Log.i(TAG, "[AR_FIX] tracking state $cameraState reason=$reason")
                                    mainScope.launch {
                                        sessionChannel.invokeMethod(
                                            "onTrackingChanged",
                                            mapOf(
                                                "state" to cameraState.name,
                                ...
                                if (showAnimatedGuide) {
                                    frame.getUpdatedTrackables(Plane::class.java).forEach { plane ->
                                        if (isStablePlaneCandidate(plane)) {
                                ...
                                val allPlanes = sceneView.session
                                    ?.getAllTrackables(Plane::class.java)
                                    .orEmpty()
                                val candidates = allPlanes.filter(::isStablePlaneCandidate).toSet()
                                planeTrackingStartedAt.keys.retainAll(candidates)
                                candidates.forEach { plane ->
                                    val firstTrackedAt = planeTrackingStartedAt
                                        .getOrPut(plane) { frame.timestamp }
                                    val stableLongEnough =
                                        frame.timestamp - firstTrackedAt >= PLANE_STABILITY_NANOS
                                    if (stableLongEnough && detectedPlanes.add(plane)) {
                                        Log.i(
                                            TAG,
                                            "[AR_FIX] plane detected type=${plane.type} " +
                                                "extent=${plane.extentX}x${plane.extentZ}",
                                        )
                                        mainScope.launch {
                        ...
                        if (node != null) {
                            ...
                            sceneView.frame?.let { frame ->
                                ...
                                planeHits.firstOrNull()?.let { hit ->
                                    Log.i(
                                        TAG,
                                        "[AR_FIX] hit type=PLANE pose=${hit.hitPose} " +
            ...
            nodesMap[name]?.let { node ->
                newTransformation?.let { transform ->
                    ...
                    node.transform = transform
                        ...
                        .toFloatArray()
                        .toTransform()
                    Log.i(TAG, "[AR_FIX] final transform ${transform.joinToString()}")
                    result.success(null)
    ...
    private fun handleGetCameraPose(result: MethodChannel.Result) {
        try {
            // ARSceneView owns Session.update(); reuse its current frame so a
            // tap cannot advance ARCore independently of rendering.
            val frame = sceneView.frame
            val cameraPose = frame?.camera?.pose
            if (cameraPose != null) {
                // Keep the platform-channel contract identical to iOS and to
                // ARHitTestResult: MatrixConverter expects a 16-value matrix.
    ...
    private fun notifyPlaneOrPointTap(hitResults: List<HitResult>) {
        mainScope.launch {
            try {
